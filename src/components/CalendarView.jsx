import { DAYS, DAY_LABELS, GRID_START_MIN, GRID_END_MIN, blockStyle, findConflicts } from '../utils/schedule';
  ...
  return (
    <div className="calendar-wrap" id="calendar-export-target">
      <div className="calendar-header">
        ...
        {DAYS.map((d) => (
          <div className="day-header" key={d}>{DAY_LABELS[d]}</div>
        ))}
      </div>
      <div
      ...
      {conflicts.size > 0 && (
        <p className="conflict-note">⚠ Overlapping time slots detected — highlighted in red.</p>
      )}
    </div>
  );
}
